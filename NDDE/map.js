...
let world; // to be assigned later
...
function loadAndDisplayData(selectedYear, currentSelectedImpact) {
  ...
  // Determine the data source: custom data (if available) or the original CSV file
  const dataPromise = customDisastersData
    ? Promise.resolve(d3.csvParse(customDisastersData))
    : d3.csv(disastersdata);

  // Load the JSON (worldmap) and the chosen data source using Promises
  Promise.all([d3.json(worldmap), dataPromise])
    .then(function (files) {
...
function updateDisplayMessage() {
  ...
  const selectedCountryLabel =
    countrySelector.options[countrySelector.selectedIndex].label || "";
  ...
  d3.json("./json/hxl-compliant-natural-disasters-decadal-data.json").then(
    function (data) {
      // Find the data for the selected country and year
      const countryData = data.find(
        (item) =>
          item["Country code"] === selectedCountryCode &&
      ...
      if (selectedImpact.includes("economic")) {
        if (value === "Unknown") {
          message = `<strong>${selectedImpact}</strong> in ${selectedCountryLabel} between ${
            selectedYear - 10
          } and ${selectedYear} was <strong>${value}</strong>`;
        } else {
          message = `<strong>${selectedImpact}</strong> in ${selectedCountryLabel} between ${
            selectedYear - 10
          } and ${selectedYear} was <strong>${value}%</strong>`;
        ...
        message = `<strong>Total ${capitalizedImpact}</strong> in ${selectedCountryLabel} between ${
          selectedYear - 10
        } and ${selectedYear} was <strong>${value}</strong>`;
      }
...
function toggleTimeLapse() {
  ...
  if (button.textContent.includes("Play Time-Lapse")) {
    ...
    button.innerHTML =
      '<i class="fa-solid fa-pause"></i>&nbsp; Pause Time-Lapse';
...
document
  .getElementById("disasterTypeSelect")
  .addEventListener("change", function () {
    // Get the current selected index from the impactSelect dropdown
    const currentSelectedImpactIndex =
      document.getElementById("impactSelect").selectedIndex;
...
document.getElementById("yearInput").addEventListener("change", function () {
  ...
  function revertAction() {
    alert(
      "Please enter a valid decade within the range 1900, 1910 ... 2000, 2010, 2020."
    );
...
document.getElementById("removeFileBtn").addEventListener("click", function () {
  ...
  loadAndDisplayData(year, selectedImpact); // Reload the map using original data
  document.getElementById("fileUploader").value = ""; // Reset the file input
});

// Event Listener for drag-and-drop zone
const dropZone = document.getElementById("dropZone");
const fileUploader = document.getElementById("fileUploader");

dropZone.addEventListener("click", function () {
  fileUploader.click(); // Open file dialog when the drop zone is clicked
});

dropZone.addEventListener("dragover", function (event) {
  event.preventDefault(); // Prevent default behavior when a file is dragged over
  dropZone.style.borderColor = "green"; // Optional: Change border color on drag over
});

dropZone.addEventListener("dragleave", function (event) {
  dropZone.style.borderColor = "#ccc"; // Optional: Revert border color on drag leave
});

dropZone.addEventListener("drop", function (event) {
  event.preventDefault(); // Prevent default behavior when a file is dropped
  dropZone.style.borderColor = "#ccc"; // Optional: Revert border color on drop

  if (event.dataTransfer.files.length) {
    fileUploader.files = event.dataTransfer.files;
    // Trigger the file upload handling logic
    handleFileUpload({ target: fileUploader });
  }
});
...
function ready(error, topo) {
  ...
  let mouseOver = function (d) {
    ...
    if (impactText === "%") {
      if (rawValue === 0 || rawValue === undefined || rawValue === null) {
        ...
        displayText =
          d.properties.name + ": " + rawValue.toFixed(2) + impactText;
      }
  ...
  function updateLegend(currentScale) {
    ...
    const legend_entry = legend
      .selectAll("g.legend_entry")
      .data(
        legendScale.range().map(function (color) {
...
function zoomToCountryContinent(countryCode) {
  ...
  for (const key in countriesInContinents) {
    if (
      countriesInContinents[key].some((country) => country.code === countryCode)
    ) {
      continent = key;
      break;
    }
